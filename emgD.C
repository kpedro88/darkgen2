...
void BookHistograms(ExRootResult *result, MyPlots *plots)
{
	...
	plots->fJetD0max = result->AddHist1D(
			"abs_jet_D0max", "abs value jet d0 max",
			"abs_value d0max 4 leading jets", "number of jet",
			50, 0.0, 1.0);

	plots->fJetD0med = result->AddHist1D(
			"abs_jet_D0med", "abs value jet d0 med",
	...
	plots->wAngleVsPt = result->AddHist2D(
        ...
        plots->GenDarkquarkPT = result->AddHist1D(
                        "GenDarkquarkPT","pt of dark quarks at gen-level",
		        "pt","number of events",
		        50,0,750.0);
         
        plots->DarkJetPT = result->AddHist1D(
         	        "DarkJetPT","pt of dark jets",
		        "pt","number of events",
		        50,0,750.0);

        plots->GenMissingET = result->AddHist1D(
		        "gen_missing_et", "Gen-level Missing E_{T}",
		        "Missing E_{T}, GeV", "number of events",
		        100, 0.0, 750.0);
	// cut flow
...
void AnalyseEvents(ExRootTreeReader *treeReader, MyPlots *plots)
{
	...
	for(entry = 0; entry < ijloop; ++entry)
	{
		...
		for(int i=0;i<ngn;i++ ) {//For the start, loop over particles and gather all relevant particles into pointtops
			...
			if((id==4900101)&&(firstdq<0)) {
				...
				prt2 = (GenParticle*) branchParticle->At(firstq);
                                prt3 = (GenParticle*) branchParticle->At(firstdq);
                                plots->GenDarkquarkPT->Fill(prt3->PT);
			}
			if((id==-4900101)&&(firstadq<0)) {
				...
				prt2 = (GenParticle*) branchParticle->At(firstaq);
                                prt3 = (GenParticle*) branchParticle->At(firstadq);
				plots->GenDarkquarkPT->Fill(prt3->PT);
			}
			if(idbg>20) {
		...
		if(branchMissingET->GetEntriesFast() > 0)
		{
		  ...
		  gmet = (MissingET*) branchGenMissingET->At(0);
                  plots->GenMissingET->Fill(gmet->MET);
		}
		...
		for(int i=0;i<iloop;i++) {
			...
			plots->fJetD0max->Fill(abs(D0Max[i]));
			plots->fJetD0med->Fill(abs(D0Med[i]));
